...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function userClickedSquare(square) {
        ...
        if (isGameOver || square.classList.contains('flag'))
            return;
        if (square.classList.contains('checked')) {
    ...
    function gameOver(losingSquare) {
    ...
    function getFlagCount(currentId) {
        const isLeftEdge = currentId % width === 0;
        const isRightEdge = currentId % width === width - 1;
        const isTopEdge = currentId < width;
        const isBottomEdge = currentId >= width * height - width;
        ...
        if (!isLeftEdge) {
            const newId = squares[currentId - 1].id;
            const newSquare = document.getElementById(newId);
            if (newSquare === null || newSquare === void 0 ? void 0 : newSquare.classList.contains('flag'))
                flagCount++;
        }
        if (!isLeftEdge && !isTopEdge) {
            const newId = squares[currentId - 1 - width].id;
